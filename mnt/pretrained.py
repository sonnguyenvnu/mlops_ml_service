...
def train_model(train_config):
    ...
    with strategy.scope():
        ...
        for e in range(num_epochs):
            ...
            # input_schema = Schema(
            #     [
            #         TensorSpec(np.dtype(np.float32), (-1, 224, 224), "input"),
            #     ]
            # )
            # signature = ModelSignature(inputs=input_schema)
            # artifact_path = f"model_epoch_{e + 1}"
            # mlflow.tensorflow.log_model(model, artifact_path=artifact_path, signature=signature, gcs_uri=gcs_uri)
        mlflow.end_run()
