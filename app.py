...
pretrained_models = ['Xception', 'VGG16']
def create_train_jobs(host, queue_name):
    q = rediswq.RedisWQ(name=queue_name, host=host)
    for model_name in pretrained_models:
        job = json.dumps({ 'model_name': model_name, 'num_epochs': 2, 'dataset_url': 'https://google.com'})
        q.put(job)
...
def train_model():
    ...
    os.system(f"python3 worker.py --redis_host={host} --queue_name={queue_name} &")
